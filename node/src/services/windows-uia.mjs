...
const UIA_SCRIPT = [
  ...
  'function Assert-LeasedWindow([long]$handle, [string]$runtimeId, [string]$className) {',
  ...
  '  if ($handles -notcontains $handle -or [CovertUiaNative]::GetOwnerPid([IntPtr]::new($handle)) -ne [uint32]$expected.pid) { throw \'UIA_WINDOW_OWNER_MISMATCH\' }',
  ...
  '        $controls = @($eligibleControls | Select-Object -First 64 | ForEach-Object { $id = [string]$_.Current.AutomationId; [pscustomobject]@{ automation_id=$id.Substring(0, [Math]::Min($id.Length, 64)); control_type=[string]$_.Current.ControlType.ProgrammaticName; enabled=[bool]$_.Current.IsEnabled; offscreen=[bool]$_.Current.IsOffscreen } })',
  '        $verified = $true',
  '        $details | Add-Member -NotePropertyName controls -NotePropertyValue $controls',
  '        $details | Add-Member -NotePropertyName controls_truncated -NotePropertyValue ($eligibleControls.Count -gt 64)',
  '      }',
  '      focus {',
  '        Assert-TargetWindow | Out-Null',
  ...
  '        $verified = ($script:focusCheckDiagnostic.foreground_matches -and $script:focusCheckDiagnostic.focused_process_id -eq [int]$expected.pid -and $script:focusCheckDiagnostic.focused_element_within_window -and $targetFocused)',
  ...
  '      }',
  '      select_file {',
  '        if (-not $payload.selection_root -or -not $payload.file_path -or -not $payload.result_window_handle) { throw \'UIA_FILE_PICKER_REQUEST_INVALID\' }',
  '        $main = Assert-LeasedWindow ([long]$payload.result_window_handle) ([string]$expected.resultWindowRuntimeId) ([string]$expected.resultWindowClassName)',
  '        $root = [IO.Path]::GetFullPath([string]$payload.selection_root).TrimEnd([IO.Path]::DirectorySeparatorChar) + [IO.Path]::DirectorySeparatorChar',
  '        $file = [IO.Path]::GetFullPath([string]$payload.file_path)',
  '        if (-not $file.StartsWith($root, [StringComparison]::OrdinalIgnoreCase) -or -not [IO.File]::Exists($file)) { throw \'UIA_FILE_OUTSIDE_ROOT\' }',
  '        $editCondition = [System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::AutomationIdProperty, \'1148\')',
  '        $edits = @($window.FindAll([System.Windows.Automation.TreeScope]::Descendants, $editCondition))',
  '        $editDiagnostics = @($edits | Select-Object -First 8 | ForEach-Object {',
  '          $candidateValuePattern = $null',
  '          $hasValuePattern = [bool]$_.TryGetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern, [ref]$candidateValuePattern)',
  '          $readOnly = if ($hasValuePattern) { [bool]$candidateValuePattern.Current.IsReadOnly } else { $null }',
  '          [pscustomobject]@{ control_type=[string]$_.Current.ControlType.ProgrammaticName; enabled=[bool]$_.Current.IsEnabled; offscreen=[bool]$_.Current.IsOffscreen; password=[bool]$_.Current.IsPassword; value_pattern=$hasValuePattern; read_only=$readOnly }',
  '        })',
  '        $nearbyElements = @($window.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition) | Where-Object { $_.Current.ControlType -eq [System.Windows.Automation.ControlType]::Edit -or $_.Current.ControlType -eq [System.Windows.Automation.ControlType]::ComboBox -or $_.Current.ControlType -eq [System.Windows.Automation.ControlType]::Pane -or $_.Current.ControlType -eq [System.Windows.Automation.ControlType]::Button } | Select-Object -First 64)',
  '        $nearbyDiagnostics = @()',
  '        foreach ($candidate in $nearbyElements) {',
  '          $valueCandidate = $null; $invokeCandidate = $null; $toggleCandidate = $null',
  '          $hasValue = [bool]$candidate.TryGetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern, [ref]$valueCandidate)',
  '          $hasInvoke = [bool]$candidate.TryGetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern, [ref]$invokeCandidate)',
  '          $hasToggle = [bool]$candidate.TryGetCurrentPattern([System.Windows.Automation.TogglePattern]::Pattern, [ref]$toggleCandidate)',
  '          $candidateId = [string]$candidate.Current.AutomationId',
  '          $idIsDiagnosticSafe = $candidateId -match \'^[A-Za-z0-9_.-]{1,64}$\'',
  '          if (-not $idIsDiagnosticSafe) { $candidateId = \'ID_REDACTED\' }',
  '          $candidateType = [string]$candidate.Current.ControlType.ProgrammaticName',
  '          $valueWritable = ($hasValue -and -not $valueCandidate.Current.IsReadOnly)',
  '          $relevant = ($candidateId -in @(\'1148\', \'1\', \'2\', \'1001\', \'System.ItemNameDisplay\', \'SearchEditBox\')) -or ($valueWritable -and $candidateType -in @(\'ControlType.Edit\', \'ControlType.ComboBox\'))',
  '          if (-not $relevant) { continue }',
  '          $parentChain = @()',
  '          if ($valueWritable) {',
  '            $parent = $candidate; $walker = [System.Windows.Automation.TreeWalker]::RawViewWalker',
  '            for ($depth = 0; $depth -lt 6 -and $null -ne $parent; $depth++) {',
  '              $parentId = [string]$parent.Current.AutomationId',
  '              if ($parentId -notmatch \'^[A-Za-z0-9_.-]{1,64}$\') { $parentId = \'ID_REDACTED\' }',
  '              $parentChain += [pscustomobject]@{ automation_id=$parentId; control_type=[string]$parent.Current.ControlType.ProgrammaticName }',
  '              $parent = $walker.GetParent($parent)',
  '            }',
  '          }',
  '          $readOnly = if ($hasValue) { [bool]$valueCandidate.Current.IsReadOnly } else { $null }',
  '          $nearbyDiagnostics += [pscustomobject]@{ automation_id=$candidateId; control_type=$candidateType; enabled=[bool]$candidate.Current.IsEnabled; offscreen=[bool]$candidate.Current.IsOffscreen; password=[bool]$candidate.Current.IsPassword; keyboard_focusable=[bool]$candidate.Current.IsKeyboardFocusable; has_keyboard_focus=[bool]$candidate.Current.HasKeyboardFocus; value_pattern=$hasValue; value_read_only=$readOnly; invoke_pattern=$hasInvoke; toggle_pattern=$hasToggle; writable_parent_chain=$parentChain }',
  '        }',
  "        $script:filePickerCheckDiagnostic = [pscustomobject]@{ stage='FILENAME_CONTROL'; filename_candidate_count=$edits.Count; filename_candidates=$editDiagnostics; nearby_control_count=$nearbyElements.Count; nearby_controls=$nearbyDiagnostics }",
  '        if ($edits.Count -ne 1 -or $edits[0].Current.IsPassword) { throw \'UIA_FILE_PICKER_CONTROL_UNAVAILABLE\' }',
  '        $fileNamePattern = $null',
  '        if (-not $edits[0].TryGetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern, [ref]$fileNamePattern) -or $fileNamePattern.Current.IsReadOnly) { throw \'UIA_FILE_PICKER_CONTROL_UNAVAILABLE\' }',
  ...
  '        $openButtons = @($window.FindAll([System.Windows.Automation.TreeScope]::Descendants, $openCondition) | Where-Object { $_.Current.ControlType -eq [System.Windows.Automation.ControlType]::Button -and $_.Current.IsEnabled })',
  "        $script:filePickerCheckDiagnostic = [pscustomobject]@{ stage='OPEN_BUTTON'; filename_candidate_count=$edits.Count; filename_pattern_available=$true; filename_read_only=[bool]$fileNamePattern.Current.IsReadOnly; open_button_count=$openButtons.Count }",
  '        if ($openButtons.Count -ne 1) { throw \'UIA_FILE_PICKER_CONTROL_UNAVAILABLE\' }',
  '        Assert-TargetWindow | Out-Null',
  '        $main = Assert-LeasedWindow ([long]$payload.result_window_handle) ([string]$expected.resultWindowRuntimeId) ([string]$expected.resultWindowClassName)',
  '        $fileNamePattern.SetValue($file)',
  '        Assert-TargetWindow | Out-Null',
  '        $edits[0].SetFocus()',
  '        if (-not $fileNamePattern.Current.Value.Equals($file, [StringComparison]::OrdinalIgnoreCase)) { throw \'UIA_FILE_PICKER_VALUE_MISMATCH\' }',
  '        Assert-TargetWindow | Out-Null',
  '        $openPattern = $null',
  '        if (-not $openButtons[0].TryGetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern, [ref]$openPattern)) { throw \'UIA_FILE_PICKER_CONTROL_UNAVAILABLE\' }',
  '        $openPattern.Invoke()',
  ...
  '          try {',
  '            $main = Assert-LeasedWindow ([long]$payload.result_window_handle) ([string]$expected.resultWindowRuntimeId) ([string]$expected.resultWindowClassName)',
  '            $statusCondition = [System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::AutomationIdProperty, [string]$payload.verify_automation_id)',
  '            $statusElements = @($main.FindAll([System.Windows.Automation.TreeScope]::Descendants, $statusCondition))',
  '            if ($statusElements.Count -eq 1) { $verified = ((Get-SafeControlValue $statusElements[0]) -ceq \'FIXTURE_FILE_ACCEPTED\') }',
  '          } catch { $verified = $false }',
  '        } while (-not $verified -and [DateTime]::UtcNow -lt $deadline)',
  '        if (-not $verified) { throw \'UIA_FILE_PICKER_POSTCONDITION_FAILED\' }',
  '        $fileBytes = [IO.File]::ReadAllBytes($file); $sha = [System.Security.Cryptography.SHA256]::Create()',
  '        try { $fileDigest = ([BitConverter]::ToString($sha.ComputeHash($fileBytes))).Replace(\'-\', \'\').ToLowerInvariant() } finally { $sha.Dispose() }',
  '        $details | Add-Member -NotePropertyName automation_id -NotePropertyValue \'1148\'',
  '        $details | Add-Member -NotePropertyName verified_by -NotePropertyValue ([string]$payload.verify_automation_id)',
  '        $details | Add-Member -NotePropertyName file_sha256 -NotePropertyValue $fileDigest',
  '        $details | Add-Member -NotePropertyName file_bytes -NotePropertyValue $fileBytes.Length',
  '      }',
  '      scroll {',
  '        $condition = [System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::AutomationIdProperty, [string]$payload.automation_id)',
  '        $elements = @($window.FindAll([System.Windows.Automation.TreeScope]::Descendants, $condition))',
...
export function validateWindowsUiaRequest(request) {
  ...
  const allowed = new Set(['action', 'pid', 'window_handle', 'lease_id', 'automation_id', 'verify_automation_id', 'expected_state', 'horizontal_percent', 'vertical_percent', 'text', 'expected_value_sha256', 'key', 'selection_root', 'file_path', 'result_window_handle', 'result_lease_id']);
  ...
  const fields = {
    ...
    select_file: ['action', 'pid', 'window_handle', 'lease_id', 'selection_root', 'file_path', 'result_window_handle', 'result_lease_id', 'verify_automation_id']
  };
  ...
  const optional = request.action === 'focus' ? ['automation_id'] : [];
  ...
  if (request.action === 'select_file') {
    if (typeof request.selection_root !== 'string' || request.selection_root.length < 2 || request.selection_root.length > 500 ||
        ...
        !validId(request.verify_automation_id)) throw new TypeError('UIA file selection requires bounded paths, result window lease, and postcondition');
  }
