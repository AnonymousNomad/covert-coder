...
const UIA_SCRIPT = [
  ...
  '        Assert-LeasedWindow ([long]$payload.result_window_handle) ([string]$expected.resultWindowRuntimeId) ([string]$expected.resultWindowClassName) | Out-Null',
...
export function validateWindowsUiaRequest(request) {
  ...
  const allowed = new Set(['action', 'pid', 'window_handle', 'lease_id', 'automation_id', 'verify_automation_id', 'verify_file_automation_id', 'verify_sha256_automation_id', 'expected_state', 'horizontal_percent', 'vertical_percent', 'text', 'expected_value_sha256', 'key', 'selection_root', 'file_path', 'result_window_handle', 'result_lease_id']);
