...
for (const pair of PAIRS) {
  try { rows[pair.model] = { a: await read(pair.a), d: await read(pair.d), e: await read(pair.e) }; } catch { /* not complete */ }
}
...
for (const m of present) {
  ...
  matrix[m] = {
    screen: { a: a.summary.passed, d: d.summary.passed, e1: e.summary.passed },
    compound: { a: c(a).map(r => r.pass), d: dCompound.map(r => r.pass), e1: e1Compound.map(r => r.pass) },
...
const familiesImproved = present.filter(m => matrix[m].compound_pass_e1 > 0).length;
let verdict = 'PENDING_MORE_MODELS';
if (present.length >= 3) {
  if (familiesImproved >= 3) verdict = 'RELATION_REPRESENTATION_DOMINANT';
  else if (familiesImproved >= 1) verdict = 'MODEL_SPECIFIC_INTERACTION';
  else verdict = 'COMPOUND_CAPABILITY_DOMINANT';
}
const out = { schema: 'condition-e-results-v1', at: new Date().toISOString(), models_present: present, matrix, compound_families_with_pass: familiesImproved, verdict,
  note: 'A and D columns frozen; E1 adds the obligation graph to the D packet on Compound rows only. Apparatus error rows recorded per model.' };
await fs.writeFile(OUT, JSON.stringify(out, null, 2));
console.log(JSON.stringify({ models_present: present, compound_families_with_pass: familiesImproved, verdict }, null, 2));
for (const m of present) console.log(m, 'screen', matrix[m].screen.a, '->', matrix[m].screen.d, '->', matrix[m].screen.e1, '| compound', JSON.stringify(matrix[m].compound.e1), '| ctrl-regressions', matrix[m].control_regressions.length);
