...
Run scripts/packaging/port-preflight.ps1 with the required product ports. It does not stop listeners. A process name or repository path in its command line is not an ownership lease; such records are labeled UNKNOWN or PATH ASSOCIATION ONLY. Exact ownership is established only by the startup probe for its directly launched process and verified descendants. The default list includes 4777, 4778, 4779, 4173, and 5173. Model-server ports are dynamically selected from model configuration and are not application startup ports. The startup probe records before/during/after port observations and uses the process-tree helper only for its own exact child, never for a listener found by this utility.
