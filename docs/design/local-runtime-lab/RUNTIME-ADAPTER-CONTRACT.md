...
An optional token-provider seam exists for protected local API versions. This slice does not read or persist credentials and does not wire a credential source. If the selected server requires an API token, protected operations return an explicit auth error; they do not inspect cloud credentials or attempt login automation. Because the API test/installation version is not live-qualified and no token provider is bound, chat, stream, load, and unload stay `PARTIAL` in the capability descriptor.
